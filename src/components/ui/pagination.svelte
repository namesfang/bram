<script lang="ts">
  ...
  let { take = 20, count = 0, queryParams = '' } = $props();
  ...
  // 当前页码
  let pageCurrent = $state(Number(searchParams.get('page') || 1))

  // 总页数
  let pageCount = $state(count > 0 ? Math.ceil(count / take) : 0);
  ...
  const d = throttle({ interval: 500 }, (el: HTMLInputElement)=> {
...
<div class="pagination">
  ...
  {#if pager.length > 1}
    {#if pageCount > pagerMax}
    <form>
      <input onkeyup={ keyup } type="number" name="page" step="1" bind:value={typing} placeholder="跳转"/>
    </form>
    {/if}
    <ul>
      {#each pager as item}
