...
export const load = async ({ url, locals })=> {
  ...
  return {
    pathname,
    tiles,
    tileId: isPositive(tileId) ? Number(tileId) : null,
...
export const actions = {
  async default({ request, locals }) {
    ...
    const tile = await prisma.branch.findFirst({
    ...
    if (typeof data.continue === 'undefined') {
      url += `?redirectURI=%2Fbranch%2F${tileId}`
    } else {
      url += `?redirectURI=%2Fbranch%2Fcreate?id=${tileId}`
    }
