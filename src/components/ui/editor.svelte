<script lang="ts">
  ...
  let {
	...
	export function dangerouslyInsertHtml(content: string) {
...
<div class="wrapper">
	<div bind:this={toolbarRef} class="toolbar"></div>
	<div bind:this={editorRef} class="editor"></div>
	{#if name.length > 0}
	<textarea bind:value {name} style:display="none"></textarea>
	{/if}
</div>
